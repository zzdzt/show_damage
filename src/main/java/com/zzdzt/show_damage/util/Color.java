...
public record Color(float r, float g, float b, float a) {
    ...
    public int getRGB() {
        return (convertComponent(r) << 16) | (convertComponent(g) << 8) | convertComponent(b);
    }

    @Override
    public @NotNull String toString() {
        if (a == 1.0F) {
            return String.format("#%02X%02X%02X", convertComponent(r), convertComponent(g), convertComponent(b));
        } else {
            return String.format("#%02X%02X%02X%02X", convertComponent(r), convertComponent(g), convertComponent(b), convertComponent(a));
        }
    }
    ...
    public static @NotNull Color valueOf(@NotNull String string) {
        ...
        if (!matcher.matches()) {
            throw new IllegalArgumentException("Invalid color code syntax: " + string);
        }
